...
| `PUBLIC_BASE_URL` | No | Publicly-reachable base URL of this API (e.g. `https://ovc.example.com`). Used to build the `profilesUrl` handed to each Encore instance the auto-scaler spawns, pointing at `GET /api/v1/profiles/index.yml` so Encore loads the operator-managed profiles from CouchDB. If unset, Encore instances fall back to `ENCORE_PROFILES_URL`. Also the origin of the `/stream/*` playback URLs `GET /api/v1/assets/:id/delivery` advertises under `DELIVERY_MODE=proxy`. |
| `DELIVERY_MODE` | No | How packaged playback is exposed: `public` (default) advertises manifest URLs on the packaged bucket's own public origin, `proxy` streams packaged objects back through this API's authorized `/api/v1/assets/:id/stream/*` route and keeps the bucket private. The two are mutually exclusive — a deployment whose selected mode is unconfigured gets `status: "not_configured"` from `/delivery` naming the variable to set, never the other mode's URL. |
| `PACKAGED_PUBLIC_BASE_URL` | No | Absolute public origin of the packaged bucket (e.g. `https://cdn.example.com/openvideocore-packaged`), used to resolve stored manifest paths into fetchable URLs. Required for playback under `DELIVERY_MODE=public`: without it `/delivery` reports `not_configured` rather than advertising a URL no player can fetch. |
