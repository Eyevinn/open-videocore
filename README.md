...
| `OVC_WORKSPACE_ID` | No | Pins the parameter-store namespace this deployment reads and writes its stack configuration under. Leave unset for normal use: the id is the tenant on the deployment's own OSC credential, written once to the deployment's own config service and read back on every later boot, so it stays the same across restarts. Set it to **move** a deployment onto a different namespace: on the next boot, if the namespace named here holds no stack configuration and exactly one other *tenant-scoped* namespace holds this deployment's, each `openvideocore/<old>/<stack>` key is copied to `openvideocore/<this>/<stack>` (originals left in place) and the new namespace is pinned, so the move survives unsetting the variable. Configuration already under `openvideocore/default/<stack>` is **not** copied and needs no copy — it is still resolved from any namespace by the legacy read fallback. Must not contain `/`, and must not be `_meta` or `storagebackends`. |
