...
| `REDIS_URL` | **Yes** | `redis://` connection URL for the shared Valkey instance used by the Encore auto-scaler (job queue, instance pool, callback poller). Without it, transcoding is disabled. Create a dedicated Valkey on OSC (`osc create valkey-io-valkey <name>`) and supply its URL here. |
