...
// unset (0 = indefinite retention). The audit store is Couch-only (issue #563);
// each tick resolves the active stack's CouchAuditRepository and skips the sweep
// when the resolved connection has no audit store (env-override no-couch /
// in-memory fallback), so no entry is ever purged in those modes.
const auditRetentionPurgeLoop = new AuditRetentionPurgeLoop({
  ...
  sweepDeps: {
    ...
    // When no audit store is available the sweep is a no-op (nothing enumerated,
    // nothing purged).
    audit: {
      listOldestPage: async (opts: { limit: number; offset?: number }) => {
        const conns = await stackResolver.resolve();
        return conns.audit ? conns.audit.listOldestPage(opts) : [];
      },
      purgeEntry: async (id: string) => {
        const conns = await stackResolver.resolve();
        return conns.audit ? conns.audit.purgeEntry(id) : false;
      }
    }
  }
});
