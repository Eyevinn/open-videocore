...
|---|---|---|---|---|
| `body_size_limit_exceeded` | 413 | `payload_too_large` | A body-size limit rejected the payload: the route's 10 GiB stream cap, Fastify's `bodyLimit`, or a proxy in front of the API enforcing its own `client_max_body_size`. | Not as-is — use the presigned or multipart tier so no single request carries the whole payload. |
...
| `unknown` | 500 | `upload_failed` | Reserved fallback. | Unclear — surface the message. |
...
plus `quota_exceeded` / `asset_not_found` / `storage_not_configured`.
...
`POST /:id/upload-complete` all return the envelope above.
